        ...
        if (activeDialog == null)
        {
            ...
            if (!isInDialogue)
            {
                ...
                ContaningDialog contaningDialog = quedDialogs[i].holder.GetComponent<ContaningDialog>();
                    if (contaningDialog.checkIfCompleted)
                    {
                        if (!QuestManager.Instance.questsExistsInCompletedQuests(contaningDialog.questsCheck))
                        {
                            quedDialogs.Remove(quedDialogs[i]);
                        }
                    }
                    else if (!QuestManager.Instance.questsExistsInCurrentQuests(contaningDialog.questsCheck))
                    {
                        quedDialogs.Remove(quedDialogs[i]);
                    }
                }
