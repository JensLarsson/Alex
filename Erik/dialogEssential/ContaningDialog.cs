...
{
    [Header("Quests")]
    public List<QuestSO> questsCheck;
    ...
    public void startConversation()
    {
        if (activateDialogWith.onFunctionCall && activateDialogWith.delay < 0 || !activateDialogWith.onFunctionCall)
        {
            ...
            {
                //startDialogueSounds();
                DialogManager.Instance.queNewDialog(diffrentStartSounds, startSoundPitchRange, newDialog, answers, doAfterDialgue, onChoseText, this.gameObject, hasBeenRead);
            }
        }
    }
    public void resetDialogue(bool chosedAnother)
    {
        ...
        if (chosedAnother || canRepeatTheDialog)
        {
            ...
            if (DialogManager.Instance.activeDialog.hasBeenRead)
            {
                newDialogue.GetComponent<ContaningDialog>().hasBeenRead = true;
            }
